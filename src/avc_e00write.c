 ...
 *                          _IsStringAlnum()
 *
 * Scan a string, and return TRUE if it contains only alnum() characters
 * or underscore.  Return FALSE otherwise.
 **********************************************************************/
static GBool _IsStringAlnum(const char *pszFname)
{
    GBool bOK = TRUE;
    ...
    {
        if (*pszFname != '_' && !isalnum(*pszFname))
            bOK = FALSE;
        pszFname ++;
    }
                                 ...
                                 const char *pszLine, AVCTableDef *psTableDef)
{
    ...
    {
      case AVCFileARC:
        strcpy(szFname, "arc.adf");
        break;
      case AVCFilePAL:
        strcpy(szFname, "pal.adf");
        break;
      case AVCFileCNT:
        strcpy(szFname, "cnt.adf");
        break;
      case AVCFileLAB:
        strcpy(szFname, "lab.adf");
        break;
      case AVCFileTOL:
        if (psInfo->nPrecision == AVC_SINGLE_PREC)
            strcpy(szFname, "tol.adf");
        else
            strcpy(szFname, "par.adf");
        break;
      case AVCFilePRJ:
        strcpy(szFname, "prj.adf");
        break;
      case AVCFileTXT:
        strcpy(szFname, "txt.adf");
        break;
      case AVCFileTX6:
    ...
    if (nStatus == 0)
    {
        ...
        if (eType == AVCFileTABLE)
            psInfo->hFile = AVCBinWriteCreateTable(pszPath, psTableDef,
                                                   psInfo->nPrecision);
        else
